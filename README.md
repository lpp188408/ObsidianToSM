...
1. 打开目标笔记。
2. 执行命令 `打开公众号发布工作台`。
3. 顶部依次提供封面、账号、刷新、复制、创建草稿、直接发布和帮助；封面按钮可选择电脑本地图片，图片独立保存在插件目录中，不写入笔记。
4. 操作栏下方是样式栏：下拉框选择排版模板，右侧色块选择颜色主题。两者可自由组合，切换后立即刷新预览。
5. 预览、刷新、复制和样式切换不需要账号。创建草稿与直接发布需要选择并配置公众号账号。
6. “创建草稿”只保存到草稿箱；“直接发布”创建草稿后提交微信发布任务，并轮询确认发布结果。微信仍在处理时以公众号后台最终状态为准。
