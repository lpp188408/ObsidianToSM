...
1. 打开目标笔记。
2. 执行命令 `打开公众号发布工作台`。
3. 顶部依次提供封面、账号、刷新、复制、创建草稿、直接发布、主题色块和帮助；封面按钮可把 Vault 图片写入笔记 frontmatter。
4. 预览、刷新、复制和主题切换不需要账号。创建草稿与直接发布需要选择并配置公众号账号。
5. “创建草稿”保存到草稿箱；“直接发布”创建草稿后提交微信发布任务。发布状态以公众号后台为准。
